...
void BlockRichardsonModel::initialize_model(std::vector<std::unique_ptr<Tensor>>& wt) {
  (void)wt;
...
void BlockRichardsonModel::initialize_weight(std::vector<std::unique_ptr<Tensor>>&) {}
