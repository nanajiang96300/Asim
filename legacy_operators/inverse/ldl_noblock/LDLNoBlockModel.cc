#include "LDLNoBlockModel.h"

#include <chrono>

void LDLNoBlockModel::initialize_weight(
    std::vector<std::unique_ptr<Tensor>>& /*weight_table*/) {
}

void LDLNoBlockModel::initialize_model(
    std::vector<std::unique_ptr<Tensor>>& weight_table) {
  (void)weight_table;
  ...
  std::string op_name = name_gen(get_name(), "LDLDecompNoBlockOp");
  auto op = std::make_unique<LDLDecompNoBlockOp>(_config, this, op_name, attrs,
                                                 /*target_core=*/0);
  ...
  spdlog::info("LDLNoBlockModel initialization time: {:2f} seconds", duration.count());
}
