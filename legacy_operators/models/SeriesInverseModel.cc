#include "SeriesInverseModel.h"

#include <chrono>

void SeriesInverseModel::initialize_weight(
    std::vector<std::unique_ptr<Tensor>>& /*weight_table*/) {
  // No external weights for this synthetic inverse test.
}

void SeriesInverseModel::initialize_model(
    std::vector<std::unique_ptr<Tensor>>& weight_table) {
  (void)weight_table;
  ...
  std::string op_name = name_gen(get_name(), "SeriesInverse");
  auto op = std::make_unique<SeriesInverseOp>(_config, this, op_name, attrs,
                                              /*target_core=*/0);
  ...
  spdlog::info("SeriesInverseModel initialization time: {:2f} seconds", duration.count());
}
