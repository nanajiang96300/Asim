...
void Simulator::cycle() {
  ...
  // Dump formula steps if requested (UOBS Phase 1)
  const char* formula_env = std::getenv("ONNXIM_FORMULA_JSON");
  if (formula_env != nullptr) {
    std::string fp(formula_env);
    if (fp.empty()) fp = "formula_steps.json";
    FormulaLogger::instance().dump_to_json(fp);
    FormulaLogger::instance().clear();
  }
}
