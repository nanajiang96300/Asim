#include "LDLDecompNoBlockAlignedModel.h"
#include "inverse/ldl_noblock/LDLDecompNoBlockAlignedOp.h"
#include <chrono>

void LDLDecompNoBlockAlignedModel::initialize_weight(
    std::vector<std::unique_ptr<Tensor>>&) {}

void LDLDecompNoBlockAlignedModel::initialize_model(
    std::vector<std::unique_ptr<Tensor>>& weight_table) {
  (void)weight_table;
  uint32_t M = 64, U = 16, batch_size = 96;
  if (_model_config.contains("matrix_m")) M = static_cast<uint32_t>(_model_config["matrix_m"]);
  if (_model_config.contains("matrix_k")) U = static_cast<uint32_t>(_model_config["matrix_k"]);
  if (_model_config.contains("batch_size")) batch_size = static_cast<uint32_t>(_model_config["batch_size"]);
  ...
  auto make_tensor = [&](const std::string& base, const std::vector<uint32_t>& shape, bool produced) -> uint32_t {
  ...
  auto op = std::make_unique<LDLDecompNoBlockAlignedOp>(_config, this,
      name_gen(get_name(), "LDLDecompNoBlockAlignedOp"), attrs, 0);
  op->set_matrix_shape(matrix_shape);
  op->add_input(h_id); op->add_input(reg_id); op->add_output(out_id);
