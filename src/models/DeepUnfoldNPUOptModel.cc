#include "DeepUnfoldNPUOptModel.h"

#include <chrono>

void DeepUnfoldNPUOptModel::initialize_weight(
    std::vector<std::unique_ptr<Tensor>>& /*weight_table*/) {
  // No external weights for synthetic optimized DeepUnfold test model.
}

void DeepUnfoldNPUOptModel::initialize_model(
    std::vector<std::unique_ptr<Tensor>>& weight_table) {
  (void)weight_table;
  ...
  if (_model_config.contains("attributes")) {
    ...
    if (_model_config["attributes"].contains("layer_group")) {
      attrs["layer_group"] = _model_config["attributes"]["layer_group"].get<std::string>();
    }
    if (_model_config["attributes"].contains("vector_interval")) {
      attrs["vector_interval"] =
          _model_config["attributes"]["vector_interval"].get<std::string>();
    }
  }
  ...
  std::string op_name = name_gen(get_name(), "DeepUnfoldNPUOptOp");
  auto op = std::make_unique<DeepUnfoldNPUOptOp>(_config, this, op_name, attrs,
                                                 /*target_core=*/0);
  ...
  spdlog::info("DeepUnfoldNPUOptModel initialization time: {:2f} seconds", duration.count());
}
