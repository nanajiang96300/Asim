...
void MMSEModel::initialize_model(
  ...
  if (_model_config.contains("attributes") &&
      _model_config["attributes"].contains("block_size")) {
    attrs["block_size"] =
        _model_config["attributes"]["block_size"].get<std::string>();
  }
  if (_model_config.contains("attributes") &&
      _model_config["attributes"].contains("solve_steps")) {
    attrs["solve_steps"] =
        _model_config["attributes"]["solve_steps"].get<std::string>();
  }
  if (_model_config.contains("attributes") &&
  ...
  // Inputs: H (batch,M,K), RegI (K,K), Y (batch,M,K);
  // Output X_hat (batch,K,K).
  uint32_t h_id = make_tensor("H", shape_H, true);
  uint32_t reg_id = make_tensor("RegI", shape_KK, true);
