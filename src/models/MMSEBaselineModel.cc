#include "MMSEBaselineModel.h"

#include <chrono>

void MMSEBaselineModel::initialize_weight(
    std::vector<std::unique_ptr<Tensor>>& /*weight_table*/) {
}

void MMSEBaselineModel::initialize_model(
    std::vector<std::unique_ptr<Tensor>>& weight_table) {
  (void)weight_table;
  ...
  const std::vector<uint32_t> shape_MU{batch_size, M, K};
  const std::vector<uint32_t> shape_UU{batch_size, K, K};
  ...
  attrs["strict_iso_lowering"] = "1";
  if (_model_config.contains("attributes") &&
      _model_config["attributes"].contains("strict_iso_lowering")) {
  ...
  uint32_t reg_id = make_tensor("Reg", shape_UU, true);
  uint32_t y_id = make_tensor("Y", shape_MU, true);
  uint32_t out_id = make_tensor("X_hat", shape_UU, false);

  std::string op_name = name_gen(get_name(), "MMSEBaselineOp");
  auto op = std::make_unique<MMSEBaselineOp>(_config, this, op_name, attrs,
                                             /*target_core=*/0);
  ...
  spdlog::info("MMSEBaselineModel initialization time: {:2f} seconds", duration.count());
}
