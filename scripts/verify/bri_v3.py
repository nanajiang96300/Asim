"""BRI v3 — per-operator verification."""
import json, numpy as np, sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from uobs_dag_executor import prim_gemm, prim_diag_add, prim_bri_precond, prim_matrix_sub, prim_matrix_add, _cplx_fp16

def fp(x):
    rp=x.real.astype(np.float16).astype(np.float64)
    ip=x.imag.astype(np.float16).astype(np.float64)
    return rp+1j*ip

def verify(formula_path):
    with open(formula_path) as f: data = json.load(f)
    meta = data.get("_metadata", {})
    U = meta.get("matrix_dim", 16)
    L = meta.get("layers", 8)
    M = 64
    
    np.random.seed(42)
    H = (np.random.randn(M, U) + 1j * np.random.randn(M, U)) / np.sqrt(2)
    A_mat = H.conj().T @ H + 0.1 * np.eye(U)
    B_ref = fp(np.linalg.inv(fp(prim_bri_precond(A_mat))))  # BRI converges to B^{-1}
    
    # BRI chain: GRAM → REG → PRECOND → iterate(BY + RESIDUAL + UPDATE) × L
    G = prim_gemm(H.conj().T, H)
    A_reg = prim_diag_add(G, 0.1)
    Bmat = prim_bri_precond(A_reg)
    
    Y = np.eye(U, dtype=np.complex128)
    I = np.eye(U, dtype=np.complex128)
    for l in range(L):
        BY = prim_gemm(Bmat, Y)
        R = prim_matrix_sub(I, BY)
        Y = prim_matrix_add(Y, R)
    
    Ainv = prim_gemm(Y.conj().T, Y)
    err = np.linalg.norm(fp(Ainv) - B_ref) / max(np.linalg.norm(B_ref), 1e-15)
    return {"error": float(err), "status": "PASS" if err < 0.25 else "FAIL", "steps": len(data["steps"])}

if __name__ == "__main__":
    r = verify(sys.argv[1] if len(sys.argv) > 1 else "/tmp/formula.json")
    print(f"BRI: err={r['error']:.4e} {r['status']}")
