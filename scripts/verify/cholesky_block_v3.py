"""Cholesky Block v3 — algorithm equivalence verification.
Block Cholesky produces same A^{-1} as full Cholesky. This script verifies
the final output. Block-specific intermediate step verification is future work."""
...
def verify(formula_path, seed=42):
    ...
    return {"error": err_dag, "status": "PASS" if err_dag < THRESHOLD else "FAIL",
            "steps": len(data["steps"]), "seed": seed, "note": "algorithm-equivalence (full Cholesky), block intermediates TBD"}
...
if __name__ == "__main__":
    ...
    print(f"Cholesky Block: err={r['error']:.4e} max_err={max_e:.4e} {r['status']}")
