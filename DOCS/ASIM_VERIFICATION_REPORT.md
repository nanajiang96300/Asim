...
## 8. B2 指令级审计结果与 SCALAR 验证策略

### 8.1 B2 审计定量结果

运行 `scripts/trace_audit.py` 对 CholeskyNoBlockBaselineOp 的 trace.csv + formula_steps.json 交叉审计：

| 指令类型 | trace 数量 | FormulaLogger 覆盖 | 覆盖率 |
|---------|-----------|-------------------|--------|
| GEMM | 192 | 192 | **100%** |
| MOVIN | 3,840 | — (数据搬运，无需声明) | N/A |
| MOVOUT | 768 | — (数据搬运，无需声明) | N/A |
| VECTOR | 3,744 | — (辅助操作，无需声明) | N/A |
| **SCALAR 合计** | **235,008** | **1,920** | **0.82%** |
| ├ SCALAR_MUL | 119,040 | — | — |
| ├ SCALAR_SUB | 65,280 | — | — |
| ├ SCALAR_DIV | 23,040 | — | — |
| ├ SCALAR_SQRT | 1,536 | — | — |
...
1. **SCALAR 是近似周期模型**：Asim 的 SCALAR Pipeline 采用固定延迟模型（SCALAR_MUL=1 cycle, SCALAR_DIV=4 cycles, SCALAR_SQRT=4 cycles），与实际硬件的标量执行单元一致。延迟参数通过 `configs/ascend_910b.json` 配置，可针对不同硬件修改。

2. **SCALAR 指令数量由算法结构决定**：给定矩阵维度 U 和分块大小 B，SCALAR 指令数由递推公式确定（如 Cholesky POTRF 需要 j 次 MUL+SUB）。只要循环边界正确（由 `/audit-operator` 审查），指令数必然正确。

3. **基地址模型的限制**：Asim 的 SPAD 使用基地址分配（整个 U×U 区域一次性 prefetch），SCALAR 指令的 `src_addrs` 和 `dest_addr` 只指向区域的基地址，不编码逐元素偏移。因此 trace.csv 无法提供"哪个 SCALAR_MUL 操作了哪个矩阵元素"的信息，指令级数值重放在架构上不可行。

**FormulaLogger 覆盖 GEMM 级语义即可**。GEMM 是唯一实际改变矩阵内容的操作（在 Python 重放中），SCALAR 指令的周期贡献通过指令计数和单发射排队模型精确建模。高层的 CHOLESKY/TRSM 原语在 DAG 执行器中重放时，调用的是 `numpy.linalg.cholesky` 和自定义前向求解——这些是完整正确的数值实现，不需要拆解到 SCALAR 粒度。
...
| 因素 | 影响 | 可控性 |
|------|------|--------|
| **基地址模型** | SCALAR 指令不携带元素级地址，无法逐元素重放 | ⬜ 架构限制 |
| **FP16 量化** | 通过 `float64→float16→float64` 往返模拟，正常信道误差 <0.01 | ✅ 已验证 |
| **GEMM 语义** | 100% FormulaLogger 覆盖，与 Python `np.matmul` 等价 | ✅ 已验证 |
| **Cholesky 精度** | 正常矩阵用 `np.linalg.cholesky`，近奇异矩阵有特征值回退 | ✅ 已处理 |
| **Barrier 时序** | 不影响数值，仅影响周期 | ✅ 周期模型负责 |

### 8.4 待修复项

| 项目 | 优先级 | 说明 |
|------|--------|------|
| 其他 5 个算子的 DAG 验证 | P1 | 当前仅 Cholesky NoBlock 完成验证 |
| LDL DAG 链路适配 | P1 | LDL 的 FormulaLogger 需加入 D, Dinv, sqrt 步骤 |
| Block 算子 (Cholesky/LDL) DAG | P2 | GEMM 密集的 DAG 结构需要适配多 GEMM 步骤 |
| Newton-Schulz DAG | P2 | 迭代结构，需验证 K 次 GEMM 链 |
| Block-Richardson DAG | P2 | 预条件器 + 迭代链，复杂度最高 |
